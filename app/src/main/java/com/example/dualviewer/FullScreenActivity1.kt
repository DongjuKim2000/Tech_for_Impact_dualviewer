...
class FullscreenActivity1 : AppCompatActivity() {
    ...
    private val hideHandler = Handler(Looper.myLooper()!!) //핸들러 초기화

    @SuppressLint("InlinedApi")
    private val hidePart2Runnable = Runnable {
        // Delayed removal of status and navigation bar 상태 바 네비게이션 바 숨기기
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        when (item.itemId) {
            ...
            R.id.menu_preference -> {
                ...
                startActivity(i) //preferences activity로 넘어감
            ...
            R.id.menu_about -> {
                ...
                val d: AlertDialog = AlertDialog.Builder(ContextThemeWrapper(this, R.style.AlertDialogTheme)) //dialog 생성
                ...
                val positiveButton: Button = d.getButton(AlertDialog.BUTTON_POSITIVE) //OK 버튼
                positiveButton.setTextColor(Color.parseColor("#515151"))

                (d.findViewById(android.R.id.message) as TextView).movementMethod = LinkMovementMethod.getInstance() //터치 이벤트 활성화 -> 하이퍼링크 가능하게
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            supportActionBar?.setDisplayHomeAsUpEnabled(true) //홈버튼 활성화
        ...
        if (ChartValue.count() == 0) //차트 초기값 넣기
        {
            for(i: Int in 1..60)
                ChartValue.add(0)
...
//        Log.d(activityName,"onResume")
        super.onResume()
        //Rest(this).getAllBG()
        Rest(this).getBG() //데이터 받아오기 (class.kt의 Rest class)
    }
    ...
    private fun toggle() { //터치했을 때 뜨는 상단바
        if (isFullscreen) {
            hide()
        } else {
            show()
    ...
    private fun show() {
        ...
        // Show the system bar 상태바 네비게이션 바 보이기
        if (Build.VERSION.SDK_INT >= 30) {
    ...
    private fun showinfo() {
        ...
        var info : String = "" //아래에 보여지는 값들 시간, IOB 등등

        if (bgTime != 0L) //데이터 가져온 후 지난 시간 계산
        {
        ...
        binding.screenBg.text = currentbg.bg //혈당 값 설정
        val direction = "${currentbg.arrow} ${currentbg.delta}"
        binding.screenDirection.text = direction //화살표 방향 설정
        binding.screenInfo.text = info //info 값 (시간, IOB 등)
        ...
        if (mins >= 16){ //데이터 가져온 후 16분 이상 지나면 텍스트에 취소선 설정 및 회색으로
        ...
        i = 0
        for(v in ChartValue.reversed()){ //i가 시간, v가 혈당 값
        ...
        lineChart.axisLeft.axisMaximum = (maxbg + 1).toFloat() //축 최댓값 설정
        lineChart.notifyDataSetChanged() //데이터 갱신
        lineChart.invalidate() //차트 갱신 및 다시 그리기
